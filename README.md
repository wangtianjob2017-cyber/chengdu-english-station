...
网站用于整理清晰、实用、可打印的英语学习资料，并提供词汇中心、资料中心、备考文章和学习诊断等入口。项目使用 HTML、CSS、JavaScript 编写，不需要后端、不需要数据库，适合部署到 GitHub Pages。

## 项目功能

- 词汇中心：集中承接同步词汇、高频词、短语、默写和作文表达升级资料。
- 资料中心：集中承接免费资料、按年级找资料、按 A/B 卷题型找资料。
- 首页展示：展示网站介绍、四大入口、热门资料、年级入口、题型入口和分数段建议。
...
- `id`：文章唯一编号，建议使用英文或拼音。
- `title`：文章标题。
- `category`：文章分类，建议使用 `题型方法`、`词汇学习`、`分数提升`、`家长指导`、`备考规划`。
- `target`：适合阅读的人群。
- `description`：文章简介。
- `date`：发布日期。
- `content`：正文内容，按小节填写。
...
- 当前主导航为：首页、词汇中心、资料中心、备考文章、学习诊断、关于本站；资料中心的“免费资料、按年级、按题型”在 `data/site-config.json` 的 `navItems.children` 中维护。
